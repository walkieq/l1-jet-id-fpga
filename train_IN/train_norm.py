...
parser.add_argument("-nmax", type=int, default=30, help="number of particle")
...
parser.add_argument("-SN", type=int, default=56, help="scale_n")
...
parser.add_argument("-nbits", type=int, default=8, help="seed")
parser.add_argument("-integ", type=int, default=0, help="seed")
parser.add_argument("-LR", type=float, default=0.0005, help="seed")
...
jetConstituent_val = np.load("data/jetConstituent_val_150_16f.npy")
target_val = np.load("data/jetConstituent_val_target_150_16f.npy")

# Restrict the number of constituents to a maximum of NMAX
nmax = args.nmax
jetConstituent     = jetConstituent[:, 0:nmax, :]
jetConstituent_val = jetConstituent_val[:, 0:nmax, :]
...
#print(jetConstituent[0])
#jetConstituent = scaler.fit_transform(jetConstituent)
#print(jetConstituent[0])
#exit()
...
X_train = jetConstituent
X_val   = jetConstituent_val
X_test  = jetConstituent_val
...
scale_e = float(args.SE/(2*nfeat))  # multiplicative factor for # hidden neurons in Edges MLP
scale_n = float(args.SN/(De+nfeat))  # multiplicative factor for # hidden neurons in Nodes MLP
...
if args.p_en:
    ...
    NSTEPS = int (( int(len(X_train) * 0.3))/args.batch)
...
outputdir = "prj/nconst{}_nbits{}_De{}_Do{}_NL{}_SE{}_SN{}_SG{}_batch{}_acc{}_P{}_Seed{}_{}".format(
    nmax,
    nbits,
    De,
    Do,
...
if args.acc:
    ...
    ls = ReduceLROnPlateau(
        monitor="val_categorical_accuracy", factor=0.2, patience=(patience/2)
    )
    ...
    # Learning rate scheduler
    ls = ReduceLROnPlateau(monitor="val_loss", factor=0.2, patience=(patience/2))
